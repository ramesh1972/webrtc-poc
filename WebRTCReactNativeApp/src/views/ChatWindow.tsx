...
import { View, Text, StyleSheet, Dimensions, Icon, Platform, PermissionsAndroid } from 'react-native';
...
class ChatWindow extends Component<Props, State> {
  constructor(props: Props) {
    ...
    this.webRTCHandler = WebRTCHandler.getInstance(this.onReceiveMessage.bind(this));
    ...
    // set basic state
		const toChannel = this.initChannelProperties(null);

		console.log('done componentDidMount');
	}

	async initChannelProperties(fromChannel: Channel) {
		// set all channels
		...
		else if (this.state.myChannel === null || this.state.myChannel === undefined) {
      ...
      myChnl = allChannels[0];
		...
		// set toChannels
    const toChannelsList = allChannels.filter(channel => channel.name !== myChnl.name);
    this.setState({ visibleChannels: toChannelsList });
    this.setState({ toChannel: toChannelsList[0] });

		const toChnl = toChannelsList[0];
    this.setState({ chatWindowChatInfo: this.getChatWindowChatInfo(myChnl, toChnl)});

		// set the initial WebRTC objects
		await this.webRTCHandler.ConnectDataChannel(myChnl.id, toChnl.id);
		const channelName = this.webRTCHandler.getCurrentDataChannelName();
	...
	setMessages(channelName: string = '') {
		...
		let messages: ChannelMessage[] = this.webRTCHandler.getChannelMessages(channelName);
  ...
  async onChannelChange(channel: Channel, channelListType: string) {
    ...
    if (channelListType === 'Users') {
      ...
      const toChannel: Channel = this.initChannelProperties(channel);
    }
     else if (channelListType === 'Channels') {
		  this.setState({ toChannel: channel });
		  this.setState({ chatWindowChatInfo: this.getChatWindowChatInfo(this.state.myChannel, channel)});

			// set the initial WebRTC objects
			console.log('Creating data channel:', this.state.myChannel.id, channel.id);
			await this.webRTCHandler.ConnectDataChannel(this.state.myChannel.id, channel.id);
	...
	onSentMessage = (channelName: string, msg: ChannelMessage) => {
		...
		console.log('OnSentMessage:', this.state.currentDataChannelName, channelName);
		if (this.state.currentDataChannelName === channelName) {
			console.log('OnSentMessage: setting currentChannelChatMessages');
			this.setState({currentChannelChatMessages: this.webRTCHandler.getChannelMessages(channelName)});
		}
	}

	// receive any type of message
  onReceiveMessage(channelName: string, messages: ChannelMessage[], type:string, msg: string): void {
	  if (this.currentDataChannelName === channelName) {
			messages = this.webRTCHandler.getChannelMessages(channelName);
	 	  this.setState({ currentChannelChatMessages: messages });
	  }
  }
	...
	// UI
  render() {
    const { selectedUser, channels, visibleChannels, myChannel, toChannel, msg, currentChannelChatMessages, webRTCCallServiceFacade } = this.state;
      ...
      <View style={styles.chatWindowContainer}>
        ...
        <View style={styles.chatWindowHeader}>
          <View style={[styles.channelList, {zIndex:101}]}>
            <ChannelList channelListType="Users" headerText="Select a User"
            selectedChannel={myChannel} channelList={channels}
            onChannelChange={this.onChannelChange}/>
          </View>
          <View style={[styles.channelList, {zIndex:100}]}>
            <ChannelList channelListType="Channels" headerText="Select a Channel"
            selectedChannel={toChannel} channelList={visibleChannels}
            onChannelChange={this.onChannelChange}/>
          </View>
         </View>

				 <View style={styles.chatBodyContainer}>
          <Text style={styles.chatWindowInfoText}>{this.state.chatWindowChatInfo}</Text>

          <View style={styles.chatWindow}>
						<ChatMessagesWindow chatMessages={currentChannelChatMessages}  dataChannelName={this.state.currentDataChannelName}/>
          </View>

         <View style={styles.chatBox}>
            <ChatBox webRTCHandler={this.webRTCHandler} onSentMessage={this.onSentMessage} currentDataChannelName={this.state.currentDataChannelName}/>
         </View>
        </View>
        </View>
    );
