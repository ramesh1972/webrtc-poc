...
export class WebRTCChannelsCollection {
    ...
    remoteStreamCallback: (stream: any) => void = () => { };

    constructor(messagecallback: (channelmessage: ChannelMessage) => void, commandcallback: (channelmessage: ChannelMessage) => void, streamcallback: (stream: any) => void) {
        this.receiveChannelMessageCallback = messagecallback;
        this.receiveSystemCommandCallback = commandcallback;
        this.remoteStreamCallback = streamcallback;
    }
    ...
    private createCallServiceFacade(dataChannelLabel: WebRTCDataChannelLabel) {
        ...
        if (callServiceFacade === null || callServiceFacade === undefined) {
            ...
            callServiceFacade = new WebRTCCallServiceFacade(dataChannelLabel, this.onReceiveMessage.bind(this), this.onReceiveSystemCommand.bind(this), this.onReceiveMediaStream.bind(this));
    ...
    private onReceiveMediaStream(stream: any): void {
        if (stream === null || stream === undefined) {
            console.error('WebRTCChannelsCollection: stream is null');
            return;
        }

        console.log('--------------> Received Media Stream in webRTCCollection:', stream);
