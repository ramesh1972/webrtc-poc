    ...
    private async createPeerConnection(): Promise<RTCPeerConnection> {
        ...
        if (this.isVideoStreaming) {
            this.pc.ontrack = (event: RTCTrackEvent) => {
                console.log('CallWindowVideo useEffect setting remote stream', event.streams[0]);
...
/*             if (this.localStream) {
                ...
                this.localStream.getTracks().forEach((track: MediaStreamTrack) => this.pc?.addStream(track, this.localStream));
            }
 */        }
    ...
    public async StartVideoStreaming(localStreamObj: any) {
        ...
        try {
            // TODO: Close gracefully if already streaming, maybe close stream first and then close peer connection
            //  if (this.IsConnected())
            //     this.CloseCall();
            ...
            // create the peer connection and start streaming
            const pc = await this.StartCall();

            if (pc) {
                console.log('WebRTCCallService: video streaming: started call successfully !!!');
                return pc;
            }
            else
